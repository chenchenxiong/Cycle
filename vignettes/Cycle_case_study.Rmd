---
title: "Case study of modelling cell type-specific lncRNA regulatory network in autism with Cycle" 
...
# ASD expression data, Values in matrix are 10x UMI counts from cellranger, log2-transformed.
# exprMatrix data could download from https://autism.cells.ucsc.edu/
...
## For user own data, users should prepare the matched lncRNA and mRNA snRNA-seq expression data, ensuring that the matrix values are 10x UMI counts from cellranger, standardized through log2-transformation, with the rows of the matrix representing samples and the columns representing genes.
...
# Discovering stable and rewired lncRNA-mRNA regulatory networks between 17 cell types
```{r, eval=FALSE, include=TRUE}
...
# Identification of rewired and stable lncRNA-target regulatory modules using MCL algorithm
```{r, eval=FALSE, include=TRUE}
...
Overlap_Cycle_network_stable_modules <- netModule(Overlap_Cycle_network_stable, method = "MCL",modulesize = 3, save = TRUE,directed = T)
```
